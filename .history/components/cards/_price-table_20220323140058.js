    ...
    return(
        <div className={data.is_recommended ? 'relative text-center bg-white rounded-b-lg' : 'relative text-center'}>
            ...
            >
                ...
                <button
                sx={styles.button}
                className={data.is_recommended ? 'btn-default bg-secondary text-white hover:bg-dark' : 'btn-primaryOutline'}
                >
                Choose Plan
                </button>
                <span className="block text-[#0F2137] mt-">{data.trial_period} days free trial</span>
            </div>
        </div>
    )
}