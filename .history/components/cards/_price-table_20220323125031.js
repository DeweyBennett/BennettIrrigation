...
export default function PriceTable({ data }) {
    ...
    return(
        <div sx={styles.card} className={data.is_recommended ? 'relative text-center bg-white rounded-b-lg' : 'relative text-center'}>
            {data.is_recommended && <span className="absolute -top-8 flex items-center justify-center rounded-t-lg bg-[#52ACFF] text-white text-sm font-bold w-full min-">Recommended</span>}
            <div
